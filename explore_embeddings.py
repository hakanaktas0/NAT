...
def experiment_digits(embedding_layer, tokenizer):
    ...
    input_ids = tokenizer(digits, add_special_tokens=False, return_tensors='pt').input_ids
...
def experiment_digits_additive(embedding_layer, tokenizer):
    ...
    additive_embeddings = [(embeddings[i] + (embeddings[i+2] - embeddings[i+1])).unsqueeze(0) for i in range(10)]
    ...
    labels = list(map(lambda x: int(x), digits))
    plt.figure(figsize=(10, 8))
    scatter = plt.scatter(reduced_embeddings[:input_ids.shape[0], 0], reduced_embeddings[:input_ids.shape[0], 1], c=labels, cmap='viridis', alpha=0.5)
    # add a text label for each point
    for i, label in enumerate(labels):
        plt.annotate(label, (reduced_embeddings[i, 0], reduced_embeddings[i, 1]), fontsize=8)
    
    
    plt.scatter(reduced_embeddings[input_ids.shape[0]:, 0], reduced_embeddings[input_ids.shape[0]:, 1], color='red', alpha=0.5)
    for i, label in enumerate([f"{i}+({i+2} - {i+1})" for i in range(10)], start=input_ids.shape[0]):
        plt.annotate(label, (reduced_embeddings[i, 0], reduced_embeddings[i, 1]), fontsize=8)
...
def show_all_vocab(embedding_layer, tokenizer):
    ...
    # Tokenize the digits separately and skip special tokens
    input_ids_digits = tokenizer(digits_10 + digits_100 + digits_1000, add_special_tokens=False, return_tensors='pt').input_ids
    ...
    # labels = list(map(lambda x: int(x), digits))
    plt.figure(figsize=(10, 8))
    scatter = plt.scatter(reduced_embeddings[embeddings_digits.shape[0]:, 0], reduced_embeddings[embeddings_digits.shape[0]:, 1], c='blue', alpha=0.2, label='All Vocab')

    plt.scatter(reduced_embeddings[embeddings_digits.shape[0]:embeddings_digits.shape[0]+10, 0], reduced_embeddings[embeddings_digits.shape[0]:embeddings_digits.shape[0]+10, 1], c='red', alpha=0.5, label='0-9')
    plt.scatter(reduced_embeddings[embeddings_digits.shape[0]+10:embeddings_digits.shape[0]+100, 0], reduced_embeddings[embeddings_digits.shape[0]+10:embeddings_digits.shape[0]+100, 1], c='orange', alpha=0.5, label='10-99')
    plt.scatter(reduced_embeddings[embeddings_digits.shape[0]+100:embeddings_digits.shape[0]+1000, 0], reduced_embeddings[embeddings_digits.shape[0]+100:embeddings_digits.shape[0]+1000, 1], c='green', alpha=0.5, label='100-999')
