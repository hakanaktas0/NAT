...
def experiment_digits(embedding_layer, tokenizer, model):
    ...
    for idx in input_ids:
        splits.append(split_embedding(idx[0], tokenizer, vocab_set, embedding_layer))
...
def experiment_embedding_vocab(embedding_layer, tokenizer, model, dataset_split_file):
    ...
    for i, idx in enumerate(input_ids):
        if (
            split := split_embedding(idx[0], tokenizer, vocab_set, embedding_layer)
        ) is not None:
            splits.append(split)
        else:
...
def main():
    ...
    rnn_model_dir = "/nfs-share/as3623/projects/L65-nat/NAT/checkpoints-20250406_213652/best_model.pt"
